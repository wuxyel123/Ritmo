...
struct FitCard<Content: View>: View {
    ...
    var body: some View {
        content
            ...
            .clipShape(RoundedRectangle(cornerRadius: RitmoTheme.cardRadius))
    }
}
        ...
        GeometryReader { geo in
            ZStack(alignment: .leading) {
                RoundedRectangle(cornerRadius: 999)
                    ...
                    .frame(height: height)
                RoundedRectangle(cornerRadius: 999)
                    .fill(value >= 1 ? RitmoTheme.positive : color)
            ...
            HStack {
                ...
                HStack(spacing: 3) {
                    Text(current, format: .number.precision(.fractionLength(fractionDigits)))
                        .fontWeight(.semibold)
                        .foregroundStyle(progress >= 1 ? RitmoTheme.positive : RitmoTheme.textPrimary)
                    Text("/ \(formattedGoal)")
                        .font(.subheadline)
                        .foregroundStyle(RitmoTheme.textSecondary)
                }
