...
struct SleepSessionCard: View {
    ...
    var body: some View {
            ...
            VStack(alignment: .leading, spacing: 14) {
                ...
                HStack(spacing: 20) {
                    ...
                    VStack(alignment: .leading, spacing: 7) {
                        ...
                        HStack(spacing: 5) {
                            Image(systemName: "clock").font(.caption).foregroundStyle(RitmoTheme.sleep)
                ...
                FitProgressBar(value: session.totalHours / 8.0, color: RitmoTheme.sleep)
        ...
        .confirmationDialog("Eliminare questa sessione di sonno?",
            ...
            Text("I dati scritti da Ritmo verranno rimossi da Apple Salute.")
        }
    }
...
struct SleepStageBar: View {
    ...
    private func legend(_ color: Color, _ label: LocalizedStringKey) -> some View {
        HStack(spacing: 4) {
            ...
            Text(label).foregroundStyle(RitmoTheme.textSecondary)
        }
    }
}
...
struct SleepDetailSheet: View {
    ...
    var body: some View {
        NavigationStack {
                ...
                VStack(spacing: RitmoTheme.gap) {
                    HStack(spacing: 24) {
                        ZStack {
                            ...
                            VStack(spacing: 0) {
                                Text("\(session.qualityScore)")
                                    ...
                                    .foregroundStyle(scoreColor)
                                Text("/ 100").font(.caption2).foregroundStyle(RitmoTheme.textSecondary)
                            }
                        }
                        ...
                        VStack(alignment: .leading, spacing: 8) {
                            ...
                            timeRow("Sveglia", session.endTime, "sun.horizon.fill", .orange)
                            HStack(spacing: 6) {
                                Image(systemName: "clock").foregroundStyle(RitmoTheme.sleep).font(.caption)
            ...
            .confirmationDialog("Eliminare il sonno di questa notte?",
                ...
                Text("I dati scritti da Ritmo verranno rimossi da Apple Salute. I dati registrati da Apple Watch rimarranno invariati.")
            }
        }
    ...
    private func stageMetric(_ value: String, _ label: LocalizedStringKey, _ color: Color, _ sub: LocalizedStringKey) -> some View {
        VStack(spacing: 3) {
            ...
            Text(sub).font(.system(size: 8)).foregroundStyle(RitmoTheme.textSecondary).multilineTextAlignment(.center)
        }
        .frame(maxWidth: .infinity)
    }
